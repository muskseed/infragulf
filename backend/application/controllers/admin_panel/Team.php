...
defined('BASEPATH') OR exit('No direct script access allowed');
 ...
 class Team extends CI_Controller
 {
 	...
 	function __construct()
	{
		...
		$this->system_module = $this->bitcoin_library->check_access_perm($this->session->userdata('usertype_id'),'Team',$this->uri->segment(4));
	...
	public function index()
	{
		...
		$this->load->view('admin_panel/team/team_list',$data);
	}
	...
	public function frontend_view($mode='',$id='')
	{
		...
		{
			$this->load->view('admin_panel/team/team_form', $data);
		}
		else if($this->system_module['view_access']==1 && $mode=='edit')
        {
            $data['edit_data'] = $this->Master_Model->get_onerecord('team',$id);
            // print_r($data);exit;   
           	$this->load->view('admin_panel/team/team_form',$data);
        }
        else
	...
	public function frontend_list()
	{
	    ...
	    $brand_value = $this->Master_Model->get_all_data('team');
	    ...
	    foreach ($brand_data as $key => $value) {
	    	$data[] = array(
                    ...
                    '<a href="'.base_url().'admin_panel/team/frontend_view/edit/'.$value['id'].'"> <button type="button" class="btn btn-primary">EDIT</button></a>
                    <a href="'.base_url().'admin_panel/team/delete_banner/'.$value['id'].'"> <button type="button" class="btn btn-danger">DELETE</button></a>
                    '
               );
	    }
	...
	public function form_action($id = '')
	{
		...
		if($_FILES['image_name']['name']!="")
    	{
			...
			if($id)
			{
				...
				// pre($final_data);die;
				$success = $this->Master_Model->do_update('team',$id,$final_data);
			}
			else
			{
				$push_array =  array(
				...
				$success = $this->db->insert('team',$final_data);
			}
