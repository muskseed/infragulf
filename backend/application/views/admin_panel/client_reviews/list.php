...
<head>  
  ...
  <title><?= COMPANY_NAME; ?> | Client Reviews List</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Client Reviews List</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
              ...
              <a href="<?php echo base_url(); ?>admin_panel/Client_reviews/frontend_view/add" class="btn btn-success" style="float: right;width: 100px;"><i class=""></i> Add</a>
            </div>
 ...
 <script type="text/javascript">
  $(document).ready(function() {
      oTableStaticFlow = $('#country_list').DataTable({
        "lengthMenu": [[25, 50, 100, -1], [25, 50, 100, "All"]],
          "ajax": {
              url : "<?php echo base_url("admin_panel/Client_reviews/frontend_list") ?>",
              type : 'POST'
          },
          "aoColumnDefs": [{
              'bSortable': false,
  ...
  $(document).on('click', '#delete_all', function () {
    ...
    if (delete_id.length === 0) {
        alert('Please select one Banner');
    }else{
      if (confirm("Are you sure?")) {
        $.ajax({
          url : "<?php echo base_url("admin_panel/Client_reviews/delete_all") ?>",
          type: "POST",
          data: {delete_id : delete_id},
          dataType: "json",
          success: function(results){
            if(results.status == 'success'){
              window.location = "<?php echo base_url('admin_panel/Client_reviews') ?>";
            }else{
              alert(results.msg);
            }
          }
...
<script type="text/javascript">
  $(document).ready(function(){
    <?php if ($this->session->flashdata('success')) { ?>
      new PNotify({
          title: 'Success!',
          text: 'Client Reviews Added Succesfully',
          type: 'success'
      });
    <?php } ?>
